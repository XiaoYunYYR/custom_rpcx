...
var DefaultOption = Option{
	...
	SerializeType:       protocol.AcePack,
...
//client.option.Heartbeat && client.option.HeartbeatInterval
...
var CircuitBreaker Breaker = NewConsecCircuitBreaker(10, 2000) //circuit.NewRateBreaker(0.95, 100)
...
type Client struct {
	option Option   // Option is struct , this is a new instance
...
func (client *Client) send(ctx context.Context, call *Call) {
	...
	isHeartbeat := call.ServicePath == "AaceCheck" && call.ServiceMethod == "check"
	serializeType := client.option.SerializeType
	//if isHeartbeat { serializeType = protocol.AcePack
	...
	if isHeartbeat {
		...
		req.SetSerializeType(protocol.AcePack)
	} else {
...
func (client *Client) input() {
	...
	for err == nil {
        ...
        isServerMessage:= res.MessageType() == protocol.Response
        if isServerMessage {
			client.mutex.Lock()
			call = client.pending[seq]
			delete(client.pending, seq)
		...
		case res.MessageStatusType() == protocol.Error:
			...
			if call.Raw {
				...
				if codec != nil {
                    //TODO ace protocol
                    acereply, arok := call.Reply.(*protocol.AceReply)
                    if arok {
                        acereply.Retcode = res.Retcode
					    _ = codec.Decode(data, acereply.Args)
                    } else {
		...
		default:
			if call.Raw {
				...
				data := res.Payload
				if len(data) > 0 && res.Retcode != -90006 {
					codec := share.Codecs[res.SerializeType()]
					if codec == nil {
						call.Error = strErr(ErrUnsupportedCodec.Error())
					} else {
                        if arok {
                            err = codec.Decode(data, acereply.Args)
                        } else {
...
func (client *Client) heartbeat() {
	...
	for range t.C {
        ...
        call := client.Go(ctx, "AaceCheck", "check", args, nil, nil)
        cancel()

        if call.Error != nil {
			log.Warnf("failed to heartbeat to %s: %v", client.Conn.RemoteAddr().String(), call.Error)
			client.Close()
        }
		...
		err := client.Call(ctx, "AaceCheck", "check", &request, &reply)
		abnormal := false
		if ctx.Err() != nil {
