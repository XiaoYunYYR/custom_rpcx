...
// first get , after if nil  then next gen
func (c *OneClient) getXClient(servicePath string) (xclient XClient, err error)  {
	c.mu.RLock()
...
func (c *OneClient) SendFile(ctx context.Context, fileName string, rateInBytesPerSecond int64, meta map[string]string) error {
    xclient, err := c.getXClient(share.SendFileServiceName)
    if err != nil {
        return err
    }
...
func (c *OneClient) Stream(ctx context.Context, meta map[string]string) (net.Conn, error) {
    xclient, err := c.getXClient(share.StreamServiceName)
    if err != nil {
        return nil, err
    }
