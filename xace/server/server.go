...
func (s *Server) handleRequest(ctx context.Context, req *protocol.Message) (res *protocol.Message, err error) {
    ...
    if mtype.UseRetcode {
	    if mtype.ArgType.Kind() != reflect.Ptr {
            res.Retcode = service.call2(ctx, mtype, reflect.ValueOf(argv).Elem(), reflect.ValueOf(replyv))
	    } else {
            res.Retcode = service.call2(ctx, mtype, reflect.ValueOf(argv), reflect.ValueOf(replyv))
        }
	    if res.Retcode == 0 {
	...
	if err != nil {
		if replyv != nil {
            ...
            if mtype.UseRetcode {
                data = xcodec.EncodeRetArgs(res.Retcode, data)
                log.Debug(data)
            }
			res.Payload = data
		}
