...
var typeOfRetcode = reflect.TypeOf((*int32)(nil)).Elem()
...
func (s *Server) registerFunction(servicePath string, fn interface{}, name string, useName bool) (string, error) {
    ...
    returnType := t.Out(0)
    useRetcode := false
    if returnType == typeOfRetcode {
        useRetcode = true
    } else if returnType != typeOfError {
		return fname, fmt.Errorf("function %s returns %s, not error", f.Type().String(), returnType.String())
	}

	// Install the methods
    ss.function[fname] = &functionType{fn: f, ArgType: argType, ReplyType: replyType, UseRetcode: useRetcode}
...
func suitableMethods(typ reflect.Type, reportErr bool) map[string]*methodType {
	...
	for m := 0; m < typ.NumMethod(); m++ {
        ...
        methods[mname] = &methodType{method: method, ArgType: argType, ReplyType: replyType, UseRetcode: useRetcode}
...
func (s *service) call2(ctx context.Context, mtype *methodType, argv, replyv reflect.Value) (ret int32) {
	defer func() {
		if r := recover(); r != nil {
			buf := make([]byte, 4096)
			n := runtime.Stack(buf, false)
	...
	returnValues := function.Call([]reflect.Value{s.rcvr, reflect.ValueOf(ctx), argv, replyv})
	// The return value for the method is an error.
	retInter := returnValues[0].Interface()
	if retInter != nil {
		return retInter.(int32)
	}
